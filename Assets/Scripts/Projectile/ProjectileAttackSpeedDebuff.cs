...
public class ProjectileAttackSpeedDebuff : ProjectileBase
{
    [SerializeField] private float moveSpeed = 5f;
    [SerializeField] private float effectRadius = 2f;
    [SerializeField] private string enemyTag = "Enemy";
    [SerializeField] private float attackSlowAmount = 0.4f; // 공격 속도 감소 비율 (0.4 = 40% 감소)
    ...
    private void ApplyEffectInArea(Vector3 centerPosition)
    {
        ...
        foreach (Collider2D collider in colliders)
        {
            if (collider.CompareTag(enemyTag))
            {
                ...
                if (enemyAttack != null)
                {
                    enemyAttack.ApplyAttackSlow(attackSlowAmount, effectDuration);
                }
            }
        }
    }
